...
set "folder="
for %%a in (c) do if not defined folder (
	pushd "%%a:\"
	for /d /r %%b in (?naconda3) do set "folder=%%b"
	popd
)
if not defined folder GOTO err_msg
echo Anaconda3 found, proceding with intall...

set TARGET=%homedrive%%homepath%\gpr_data_vis
...
start "" /b/d"%folder%" Scripts\activate.bat
call CD /D "%TARGET%"
call conda env create --file datavis.yaml --prefix="%TARGET%\gprenv"
echo CD /D "%TARGET%" >> "%TARGET%\dzt_visualizer.bat"
echo start "" /b/d"%folder%" Scripts\activate.bat >> "%TARGET%\dzt_visualizer.bat"
